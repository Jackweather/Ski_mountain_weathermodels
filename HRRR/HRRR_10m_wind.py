...
# Wind speed forecast for Gore Mountain using HRRR 10m wind data
import os
import requests
import datetime
import pygrib
...
def download_grib_file_task(args):
    ...
    if response.status_code == 200:
        grib_dir = os.path.join("HRRR", "Gore", "grib_files_wind")
...
# -----------------------------
# EXTRACT WIND SPEED AT GORE
# -----------------------------
def extract_gore_wind(grib_file, gore_coords):
    with pygrib.open(grib_file) as grbs:
        for grb in grbs:
            if (grb.name == "Time-maximum 10 metre wind speed" or grb.shortName == "10si"):
                lats, lons = grb.latlons()
                values = grb.values
                # Convert m/s to mph: 1 m/s = 2.23694 mph
                wind_mph = values * 2.23694
                dist = np.sqrt((lats - gore_coords[0])**2 + (lons - gore_coords[1])**2)
                min_idx = np.unravel_index(np.argmin(dist), dist.shape)
                gore_val = wind_mph[min_idx]
                return round(gore_val, 2)
    return None
...
# CHART GENERATION
# -----------------------------
def generate_wind_chart(forecast_data, output_dir, start_date, start_hour, run):
    winds = [d["wind_mph"] for d in forecast_data]
    times = [convert_to_12_hour((start_hour + i) % 24) for i in range(len(forecast_data))]
    plt.figure(figsize=(10,6))
    plt.plot(range(len(winds)), winds, marker="o")
    run_label = run
    plt.title(f"Gore Mountain 10m Wind Speed Forecast (HRRR model - Run {run_label}z)", fontsize=16)
    plt.ylabel("10m Wind Speed (mph)", fontsize=14)
    ...
    if os.path.exists(chart_path):
        ...
        import shutil
        shutil.move(chart_path, os.path.join(old_dir, "gore_mountain_10m_wind.png"))
...
# MAIN FUNCTION
# -----------------------------
def generate_gore_wind_forecast():
    ...
    for hour, grib_file in enumerate(grib_files):
        if grib_file:
            wind_val = extract_gore_wind(grib_file, gore_coords)
            ...
            forecast_data.append({
                ...
                "wind_mph": wind_val if wind_val is not None else None
            })
        else:
            local_hour_24 = (start_hour + hour) % 24
            local_hour_12 = convert_to_12_hour(local_hour_24)
    ...
    generate_wind_chart(forecast_data, output_dir, date, start_hour, run)
    grib_dir = os.path.join("HRRR", "Gore", "grib_files_wind")
