...
def extract_gore_tmp(grib_file, gore_coords):
    with pygrib.open(grib_file) as grbs:
        for grb in grbs:
            if (grb.name == "2 metre temperature" or grb.shortName == "2t"):
                lats, lons = grb.latlons()
                values = grb.values
                # Convert K to F: (K - 273.15) * 9/5 + 32
                tmp_f = (values - 273.15) * 9/5 + 32
                dist = np.sqrt((lats - gore_coords[0])**2 + (lons - gore_coords[1])**2)
                min_idx = np.unravel_index(np.argmin(dist), dist.shape)
                gore_val = tmp_f[min_idx]
...
def generate_tmp_chart(forecast_data, output_dir, start_date, start_hour, run):
    tmps = [d["tmp_f"] for d in forecast_data]
    times = [convert_to_12_hour((start_hour + i) % 24) for i in range(len(forecast_data))]
    plt.figure(figsize=(10,6))
    plt.plot(range(len(tmps)), tmps, marker="o")
    run_label = run
    plt.title(f"Gore Mountain 2m Temperature Forecast (HRRR model - Run {run_label}z)", fontsize=16)
    plt.ylabel("2m Temperature (°F)", fontsize=14)
    ...
    if os.path.exists(chart_path):
        ...
        import shutil
        shutil.move(chart_path, os.path.join(old_dir, "gore_mountain_2m_tmp.png"))
...
def generate_gore_tmp_forecast():
    ...
    for hour, grib_file in enumerate(grib_files):
        if grib_file:
            tmp_val = extract_gore_tmp(grib_file, gore_coords)
            ...
            forecast_data.append({
                ...
                "tmp_f": tmp_val if tmp_val is not None else None
            })
        else:
            local_hour_24 = (start_hour + hour) % 24
            local_hour_12 = convert_to_12_hour(local_hour_24)
